    ...
    def visit_binary_operator(self, node):
        ...
        if is_left_op:
            if node.type <= binop.T_LOGICAL_AND:
                left_parentheses = (
                    node.left.type <= node.type or
                    (node.left.type <= binop.T_LOGICAL_AND and
                     node.type <= binop.T_LOGICAL_AND)
        ...
        if is_right_op:
            if node.type <= nodes.BinaryOperator.T_LOGICAL_AND:
                right_parentheses = (
                    node.right.type <= node.type or
                    (node.right.type <= binop.T_LOGICAL_AND and
                     node.type <= binop.T_LOGICAL_AND)
