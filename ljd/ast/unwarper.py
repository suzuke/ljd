...
def _make_explicit_subexpressions(parts):
    ...
    while i < len(parts) - 1:
        ...
        if operator < last_operator:
            ...
            if operator > last_operator and\
                    (i - subexpression_start) % 2 != 0:
    ...
    for start, end in loops:
        if start.index in (outer_start_index, current_start_index):
            ...
            if current_end is not None and\
                    current_start_index < start.index and\
                    current_end.index >= end.index:
...
def _is_flow(warp):
    return isinstance(warp, nodes.UnconditionalWarp) and\
